...
class HotelManagementSystem:
    ...
    def add_reservation(self):
        # 创建添加预订的顶层窗口
        add_reservation_win = tk.Toplevel(self.root)
        ...
        def save_reservation():
            ...
            try:
                ...
                def check_room_availability(room_number, start_date, end_date=None):
                    ...
                    try:
                        # 开始事务
                        connection = get_db_connection()  # 假设这是获取数据库连接的函数
                        ...
                        if not room:
                            messagebox.showerror("错误", f"房间 {room_number} 不存在")
                            return False

                        if room[1] != "available":
                            messagebox.showerror("错误", f"房间 {room_number} 当前状态为: {room[1]}")
                            return False
                        ...
                        if end_date:
                            ...
                            """, (room[0], start_date, start_date, end_date, end_date, start_date, end_date))
                        else:
                            # 单日预订检查
                            cursor.execute("""
                                SELECT COUNT(*) FROM bookings 
                            ...
                            """, (room[0], start_date, start_date))

                        booking_count = cursor.fetchone()[0]
                        if booking_count > 0:
                            messagebox.showerror("错误", f"房间 {room_number} 在所选日期已被预订")
                            return False

                        # 检查是否达到最大预订限制
                        cursor.execute("SELECT COUNT(*) FROM bookings WHERE customer_id = ?", (current_customer_id,))
                        if cursor.fetchone()[0] >= MAX_BOOKINGS_PER_CUSTOMER:
                            messagebox.showerror("错误", "您已达到最大预订数量限制")
                            return False

                        # 锁定房间（乐观锁或悲观锁实现）
                        # 示例：悲观锁 - 在事务中锁定该行
                        cursor.execute("SELECT id FROM rooms WHERE room_number = ? FOR UPDATE", (room_number,))
                        ...
                        if 'connection' in locals():
                            connection.rollback()
                        messagebox.showerror("错误", f"检查房间可用性时发生错误: {str(e)}")
                        return False
                    finally:
                        # 关闭数据库连接
                        if 'connection' in locals():
        ...
        ttk.Button(add_reservation_win, text="保存", command=save_reservation).grid(row=6, column=0, columnspan=3,
                                                                                    pady=10)
